...
static int ept_lookup_gpa(epte_t* eptrt, void *gpa, 
	...
	eptrt[PML4(gpa)] = PTE_ADDR(eptrt[PML4(gpa)]) | __EPTE_FULL;
	pdpe_t* pdpe = (pdpe_t*) KADDR(PTE_ADDR(eptrt[PML4(gpa))]);
	pdpe = &pdpe[PDPE(gpa)];
	*pdpe = PTE_ADDR(*pdpe) | __EPTE_FULL;
	pde_t* pde = (pde_t*) KADDR(PTE_ADDR(*pdpe));
	pde = &pde[PDX(gpa)];
	*pde = PTE_ADDR(*pde) | __EPTE_FULL;
	if(epte_out){
		*epte_out = (epte_t*) pte;
	}
	return 0;
...
int ept_map_hva2gpa(epte_t* eptrt, void* hva, void* gpa, int perm, 
	...
	epte_t* ept;
	int e;
	if((e = ept_lookup_gpa(eptrt, gpa, 1, &ept)) != 0) {
		cprintf("Some went wrong in ept_lookup_gpa...%e\n", e);
		return -E_INVAL; // What else can I return?
	} else {
		// ept now contains the pointer to the mapping.
		if(*ept == 0) {
